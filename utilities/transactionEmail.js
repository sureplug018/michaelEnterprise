...
module.exports = class OrderEmail {
  ...
  async sendTransactionNotification() {
    try {
      await this.send('transactionNotification', 'New Transaction notification');
    } catch (error) {
  ...
  async sendTransactionConfirmationEmail() {
    try {
      await this.send('transactionConfirmed', 'Transaction confirmation notification');
    } catch (error) {
      // Handle error for sendPasswordReset method
  ...
  async sendTransactionDeclined() {
    try {
      await this.send('transactionDeclined', 'Transaction declined');
    } catch (error) {
      // Handle error for sendPasswordReset method
      console.error('Sending transaction cancellation email failed:', error);
      // Optionally, you can throw the error again for higher-level handling
      throw new Error('Failed to send transaction cancellation email');
    }
  }
};
