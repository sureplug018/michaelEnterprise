...
exports.addToCartFromWishlist = async (req, res) => {
  ...
  try {
    const product = await Wishlist.findOne({ user, productId });

    if (!product) {
      return res.status(404).json({
        status: 'fail',
        message: 'Product not in you wishlist',
      });
    }

    // check if product already exists in user cart
    const productAlreadyInCart = await Cart.findOne({ user, productId });
    ...
    const toCart = await Cart.create({
      user,
      productId,
    });
